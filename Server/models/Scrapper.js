...
class Scrapper{
    static _uri = "https://www.maannews.net";
    ...
    static async scrap(pageData) {
        ...
        await page.goto(`${Scrapper._uri}/${pageData.type}?page=${pageData.page}`);

        const news = await page.evaluate(() => {
            return Array.from(document.querySelectorAll("div.list-6.list-6-3.mt-20m > div > div > a")).map(paper => {
                const link = paper.getAttribute('href');
                return {
                    link,
    ...
    async lastPage(){
        ...
        await page.click(".pagination-container > ul > li:last-child > a");
        await page.waitForSelector(".pagination-container > ul > li:nth-child(5) > a");
        const lastPageNumber = await page.$eval("div.pagination-container > ul > li:nth-child(5) > a", el => el.text)
